...
// Prototypes
static int namecmp(const char *s, const char *t, int n);
static struct dirent *eget(struct dirent *parent, char *name);
int enext(struct dirent *dp, struct dirent *ep, uint off, int *count);
int dirnext(struct dirent *dp, int user_dst, uint64 dst, uint off, uint n);
...
static uint32 read_fat(uint32 cluster)
{
    ...
    struct buf *b = bread(ROOTDEV, fat_sec);
...
{
    if (off + n < off) return 0;
    if (entry->attribute & ATTR_DIRECTORY) {
        return dirnext(entry, user_dst, dst, off, n);
    }
    ...
    if (name && name[0] != 0) {
        for (ep = root.next; ep != &root; ep = ep->next) {          // LRU algo
            if (ep->valid == 1 && ep->parent == parent
                && namecmp(ep->filename, name, FAT32_MAX_FILENAME) == 0) {
                if (ep->ref++ == 0) {
                    ep->parent->ref++;
                }
                release(&ecache.lock);
...
int dirnext(struct dirent *dp, int user_dst, uint64 dst, uint off, uint n)
{
    ...
    int count = 0;
    uint32 real_off = off;
    union dentry de, prev_de;
    
    if (n < 32) return 0;

    // Read the current entry
    int off_in_clus = reloc_clus(dp, real_off, 0);
    if (off_in_clus == -1) return 0;
    if (rw_clus(dp->cur_clus, 0, 0, (uint64)&de, off_in_clus, 32) != 32) return 0;

    if (de.lne.order == END_OF_ENTRY) return 0;
    if (de.lne.order == EMPTY_ENTRY) {
        struct { uint16 inum; char name[30]; } user_de = {0};
        either_copyout(user_dst, dst, &user_de, 32);
        return 32;
    }
    ...
    if (de.lne.attr == ATTR_LONG_NAME) {
        ...
        // It's an SFN. Check if preceded by LFN.
        if (real_off == 0) {
            is_start = 1;
        } else {
            int prev_off_in_clus = reloc_clus(dp, real_off - 32, 0);
            if (prev_off_in_clus != -1 && 
                rw_clus(dp->cur_clus, 0, 0, (uint64)&prev_de, prev_off_in_clus, 32) == 32) {
                if (prev_de.lne.attr != ATTR_LONG_NAME) {
                    is_start = 1;
                }
            } else {
                is_start = 1;
    ...
    if (is_start) {
        ep = eget(dp, "");
        ep->valid = 0;
        if (enext(dp, ep, real_off, &count) == 1) {
            user_de.inum = (uint16)ep->first_clus;
            strncpy(user_de.name, ep->filename, 30);
        }
        eput(ep);
    }

    if (either_copyout(user_dst, dst, &user_de, 32) != -1) {
        return 32;
    }
    return 0;
}
...
static int namecmp(const char *s, const char *t, int n)
{
    while (n > 0 && *s && (lowcase(*s) == lowcase(*t))) {
        s++;
        t++;
        n--;
    }
    if (n == 0) return 0;
    return (uchar)lowcase(*s) - (uchar)lowcase(*t);
}
...
struct dirent *dirlookup(struct dirent *dp, char *filename, uint *poff)
{
    ...
    while ((type = enext(dp, ep, off, &count)) != -1) {
        ...
        } else if (namecmp(filename, ep->filename, FAT32_MAX_FILENAME) == 0) {
            ep->parent = edup(dp);
            ep->off = off;
            ep->valid = 1;
            return ep;
